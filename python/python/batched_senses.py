...
for i in range(nenc):
	...
	diag = torch.real(tkbn.calc_toeplitz_kernel(omega=coord_cu, im_size=im_size)).cpu()
...
#images1 = hasty_sense.nufft21(coord_cu, images[0,...].to(cudev)).cpu() / NF
images1 = toep_sense(images)

coord = coord.numpy()

#images2 = finufft.nufft3d2(coord[0,:], coord[1,:], coord[2,:], images[0,0,...].numpy())
#images2 = finufft.nufft3d1(coord[0,:], coord[1,:], coord[2,:], images2, (N1,N2,N3))


i1 = images1[0,...].numpy().flatten()
#i2 = images2.flatten() / NF
