...
def run_from_difference(settings: RunSettings):
	...
	for frame in range(nframes):
		for enc in range(5):
			kdata = SenseT(coord_vec[frame*5 + enc].to(cudev), smaps_cu, coils).apply(images_full[enc].to(cudev)).cpu()
	...
	mean_images = torch.empty((nframes,5) + settings.im_size, dtype=torch.complex64)
	for frame in range(nframes):
		for enc in range(5):
			mean_images[frame,enc,...] = images_full[enc,0,...]
