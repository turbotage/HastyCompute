...
static PetscErrorCode cuda_mult(PetscMat A, PetscVec x, PetscVec y)
{
    ...
    // Clone immediately so matvec doesn't race with PETSc after RestoreArrayRead.
    auto xt = petsc_to_op(xd, nx, ctx->op->dtype(), dev).clone();
                             ...
                             Tensor& U, Tensor& Vh, bool cuda, i32 cuda_dev)
{
    if (!cuda) {
        ...
        auto u_src = Tensor::from_blob(
            ...
            {m}, eScalarType::ComplexDouble, dev).clone();  // clone completes before restore
        ...
        auto v_src = Tensor::from_blob(
            ...
            {n}, eScalarType::ComplexDouble, dev).clone();
                       ...
                       double tol, i64 max_its)
{
    ...
    // Enforce SLEPc constraint: ncv ≤ k + mpd, and ncv ≤ min(m, n).
    i64 dim = std::min(m, n);
    if (mpd < 0) mpd = std::max(k, (i64)10);
    if (ncv < 0) ncv = std::min(k + mpd, dim);
    ncv = std::min(ncv, k + mpd);
    ncv = std::min(ncv, dim);
